...
The key approximation used here is unchanged in spirit: the optimized center
source defines a reference rotor orientation, and each lattice site reuses that
pattern after a rigid rotation that aligns the **center** line of sight with the
**local** line of sight. The interferometer arms are fixed globally along ``+x``
and ``+y`` in ``metricCalculate.py``, so arm directions are no longer rotated per
source.
"""
...
import bestPosition
from metricCalculate import ExperimentConfig, spherical_unit_vector

DATA_DIR = Path(__file__).resolve().parent / "Data"
BEST_POSITION_FILE = DATA_DIR / "bestPosition.txt"
DEFAULT_OUTPUT_FILE = DATA_DIR / "source_array_distribution.csv"
TWO_PI = 2.0 * np.pi
...
class ArrayContext:
    ...
    reference_geometry: RelativeGeometry
...
def solve_best_geometry(recompute: bool = False) -> RelativeGeometry:
    ...
    if not recompute and BEST_POSITION_FILE.is_file():
        try:
            angles = _parse_best_position_text(BEST_POSITION_FILE.read_text(encoding="utf-8"))
        except OSError:
            angles = None
    ...
    if angles is None:
        ...
        initial_phi_src = 0.0
        initial_theta_rot = float(np.pi / 2.0)
        initial_phi_rot = 0.0
...
def build_array_context(
    ...
    **Detector frame placement.** Sources live near the origin; the interferometer
    ...
    """
    config = ExperimentConfig()
    spacing = config.D if spacing is None else float(spacing)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--spacing",
        type=float,
        default=ExperimentConfig.D,
        help="Center-to-center spacing of neighboring sources in meters. Default: source diameter D.",
    )
    parser.add_argument(
        "--theta-array",
        type=float,
...
def main() -> None:
    ...
    Typical usage:
        python sourceArray.py --num-sources 10000000 --output Data/sourceArray.csv
    """
