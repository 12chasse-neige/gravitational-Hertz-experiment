...
from ghe.finite_distance import (michelson_response, newtonian_acceleration,
                                  quadrupole_field, rotor_quadrupole_phasor)
...
def test_static_field_is_newtonian_and_curvature_is_tidal_gradient():
    ...
    jacobian = np.column_stack([
        (newtonian_acceleration(x+step*u, Q, G=1)
         - newtonian_acceleration(x-step*u, Q, G=1))/(2*step)
        for u in np.eye(3)])
    np.testing.assert_allclose(field["tidal"], -jacobian, rtol=5e-9, atol=1e-13)


@pytest.mark.parametrize("kr", [0.001, 0.075, 1., 10., 100.])
def test_vacuum_trace_and_harmonic_constraint(kr):
    ...
    step = 1e-5/max(1., omega)
    divergence = -1j*omega*bar_up(x)[0, :]
    for j, u in enumerate(np.eye(3)):
        divergence += (bar_up(x+step*u)[j+1, :]-bar_up(x-step*u)[j+1, :])/(2*step)
...
@pytest.mark.parametrize("omega", [0.005, 0.5, 5., 30.])
def test_finite_arm_response_agrees_in_two_gauges(omega):
    response = michelson_response([2.1,3.2,4.3], Q, omega,
                                 arm_length=1, G=1, c=1, quadrature_order=96)
    np.testing.assert_allclose(response["total"],response["curvature_total"],
                               rtol=3e-12,atol=0)
...
def test_rotor_phasor_matches_independent_time_domain_quadrupole():
    from ghe.metric import calculate_whole_tensor
    from ghe.geometry import rotation_body_to_detector
    cfg=SourceConfig()
    rot=rotation_body_to_detector(0.4,0.8)
    phase=2*np.pi*np.arange(64)/64
    values=np.array([rot@calculate_whole_tensor(t/cfg.gw_angular_frequency,cfg)@rot.T
                     for t in phase])
    direct=2*np.mean(values*np.exp(1j*phase)[:,None,None],axis=0)
    np.testing.assert_allclose(rotor_quadrupole_phasor(0.4,0.8,config=cfg),direct,
