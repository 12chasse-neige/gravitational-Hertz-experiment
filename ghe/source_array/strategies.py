...
``chunk_anchor``
    ...
    nearby sources; each response phase is evaluated separately.
"""
...
def optimize_rotor_for_fixed_source(
    ...
    _, _, theta_rot, phi_rot = scipy_gradient_descent(
        lambda ts, ps, tr, pr: scaled_spherical_function(
            ts, ps, tr, pr, config=replace(context.config, R=distance)
        ),
        theta_src,
        phi_src,
...
def exact_anchor_for_source_index(
    ...
    Returns the optimized rotor-axis vector and source-to-detector unit vector.
    Anchors carry orientation only; response phases are evaluated per row.
    """
    ...
    theta_rot, phi_rot = optimize_rotor_for_fixed_source(
        context, theta_src, phi_src, distance
    )
    rot_axis_vec = context.geometry_vector(theta_rot, phi_rot)
    return rot_axis_vec, u_src_to_detector
...
def approximate_chunk_center_parameters(
    ...
    Each group pays for one rotor optimization. Other rows rigidly transport
    that axis. build_chunk evaluates every resulting complete response phase.
    """
    ...
    while cursor < len(indices):
        ...
        center_rot_axis, center_u_src_to_detector = exact_anchor_for_source_index(
            context, center_index
        )
        ...
        theta_rot[local_slice], phi_rot[local_slice] = cartesian_to_spherical(
            local_rot_axis
        )

        # This strategy approximates orientation only. The caller evaluates
        # the complete detector phase for EVERY row after transporting axes.
        cursor = local_stop
